...
class PostProcessVP():
    ...
    def apply(self, vp, loss, grad):
        ...
        """
        # Scale to obtain velocity gradient. Devito returns dL/dm for the
        # squared slowness m = 1 / vp**2, so the chain rule to velocity is
        # dL/dvp = dL/dm * dm/dvp = dL/dm * (-2 / vp**3)
        grad = - 2. * grad / (vp ** 3)
